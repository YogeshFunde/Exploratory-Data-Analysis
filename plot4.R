...
par(mfrow=c(2,2)) # setting global parameter as a image panel of 2 x 2

#plotting first plot at 1,1
plot(x=mydates, y=as.numeric(Data$Global_active_power), type="l", ylab = "Global Active Power", xlab = "")

#plotting second plot at 1,2
plot(x=mydates, y=as.numeric(Data$Voltage), type="l", ylab = "Voltage", xlab = "datetime")

#plotting third plot at 2,1
...
#plotting  fourth plot at 2,2
plot(x=mydates, y=as.numeric(Data$Global_reactive_power), type="l", ylab = "Global_reactive_power", xlab = "datetime", ylim = c(0.0,0.5))

dev.copy(png, file="plot4.png") #sending the file to png device
dev.off()